package com.syntepro.appbeneficiosbolivia.ui
...
import com.syntepro.appbeneficiosbolivia.entity.service.VersionsRequest
            ...
            try {
                ...
                when {
                    response.isSuccessful -> {
                        ...
                        if (ret.isSuccess) {
                            ret.data?.let {
                                val contains = it.any { v ->
                                    v.versionNumber == currentVersion
                                }
                                if (contains) updateUI()
        ...
        call.enqueue(object : Callback<ResponseBody?> {
            override fun onResponse(call: Call<ResponseBody?>?, response: Response<ResponseBody?>) {
                if (response.code() == 200) {
                    ...
                    } catch (e: java.lang.Exception) { updateUI() }
                } else updateUI()
            }

            override fun onFailure(call: Call<ResponseBody?>?, t: Throwable?) { updateUI() }
        })
    }
